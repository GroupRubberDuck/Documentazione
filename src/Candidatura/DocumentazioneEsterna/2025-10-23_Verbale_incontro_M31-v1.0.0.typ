...
#registroModifiche((
  ([0.1.0],[2025-10-28],[Davide Testolin],[Filippo Guerra],[Prima stesura],),
  ([1.0.0],[2025-10-29],[Cristian Pirlog],[Cristian Pirlog],[Approvazione])
))
]
...
#enfasi[Risposta 1]
Ci potrebbe essere l’integrazione con sistemi di analisi avanzata e algoritmi predittivi ma si tratta di un *"nice to have"*. I dati potrebbero passare attraverso una serie di stadi di "processing" prima di arrivare al cliente finale.
#line(length: 100%)


#enfasi[== Segregazione tenant ]
Nella pratica in che modo e quali strumenti vengono utilizzati per realizzare una segregazione tra tenant?

#enfasi[Risposta 2]
*Sostanzialmente ogni tenant è un cliente*. Tenere i dati in un singolo DB potrebbe essere problematico se si hanno ad esempio centinaia di migliaia di sensori che generano dati contemporaneamente. Quindi sia a livello logico di codice che a livello fisico di DB separati (oppure vedi la pagina #link("https://www.postgresql.org/docs/current/ddl-rowsecurity.html")[Row Security Policy] di Postgres).
...
#enfasi[Risposta 3]
Direi che questi dettagli potremo definirli insieme piu avanti, poiché in uno dei primi incontri sarà necessario anche discutere e concordare i requisiti elencati. Tali requisiti non sono scolpiti nella pietra. Sta a voi valutarli e segnare tempestivamente eventuali difficoltà nel realizzarli. In sostanza, ci aspettiamo di avere una dashboard dove possiamo vedere i dati trasmessi dai sensori attivi in real time, magari con qualche filtro, per il tenant corrente.
...
== Stato dell'Arte
È possibile avere delle dritte per il requisito analisi dello stato dell’arte? Esempi di piattaforme da analizzare? Quale dovrebbe essere la struttura approssimativa?
#v(1.2cm)
#enfasi[Risposta 8]
...
#enfasi[Risposta 12]
Sì, MQTT, purché non produca ritardi, in quel caso meglio HTTPS.


#line(length: 100%)

== Persistenza dei dati
Nel capitolo 3.3 è scritto: “Include un sistema di persistenza temporanea dei dati, con possibilità di estensione a database a lungo termine.” Quindi nell’MVP è richiesto che l’applicazione salvi i dati in un database a lungo termine?

#enfasi[Risposta 13]
Sì, è richiesto che nell'MVP l'applicazione salvi i dati in un database a lungo termine.
