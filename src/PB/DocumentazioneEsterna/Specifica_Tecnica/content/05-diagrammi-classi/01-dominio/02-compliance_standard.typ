...
_ComplianceStandard_ è l'entità radice che rappresenta una norma o standard di conformità. È composta da una collezione di requisiti validati univocamente. La classe è progettata per essere immutabile garantendo l'integrità dei dati tramite incapsulamento.
...
_Requirement_ è un'entità immutabile che definisce un singolo requisito di conformità, includendo le sue dipendenze e l'albero decisionale per valutarlo. Applica regole di business stringenti, come il fallimento automatico se il verdetto è "Non Applicabile" (NA) e manca la giustificazione testuale.
...
#figure(
  image("/src/PB/DocumentazioneEsterna/Specifica_Tecnica/content/05-diagrammi-classi/uml/png/Dominio/Node.drawio.png", width: 30%),
  caption: [Node]
) <fig-node-domain>

*Descrizione*

_Node_ è un'interfaccia che definisce il contratto base per tutti gli elementi che compongono un albero decisionale.
...
_DecisionNode_ è un oggetto immutabile che implementa _Node_. Rappresenta uno snodo dell'albero che pone una domanda e biforca il percorso di valutazione.
...
_LeafNode_ è un oggetto
 immutabile che implementa _Node_ e rappresenta la terminazione di un percorso decisionale (foglia), contenendo il risultato finale della valutazione.
