...
- *Scenario Principale*:#pad(left: 1em)[
    ...
    + Il sistema apre l'interfaccia di selezione, mostrando file nei formati supportati
...
- *Scenari Alternativi*:#pad(left: 1em)[
    #v(-0.5em)
    #terms(separator: ": ", ([UC1.1], [File di ingresso vuoto]), ([UC1.2], [File di ingresso di dimensione eccessiva]), ([UC1.3], [Una o più funzionalità non sono state lette correttamente]))
...
- *Scenario Principale*:#pad(left: 1em)[
    ...
    + Il sistema notifica l'errore all'utente con un messaggio;
...
- *Precondizioni*: L'utente ha selezionato un file per l'importazione;
- *Postcondizioni*: L'utente viene informato dell'errore e può tentare una nuova importazione;
- *Trigger*: Il sistema rileva che il file è troppo grande;
- *Scenario Principale*:#pad(left: 1em)[
    #v(-0.5em)
    + Il sistema apre il file selezionato;
  ...
  ==== UC1.3: Lettura non corretta di singole funzionalità
- *Precondizioni*: L'utente ha selezionato un file per l'importazione;
